const Card = ({image, name, price, popular, rating, votes, inStock}) => {
    ...
    return (
        <div className="flex flex-col rounded-md overflow-hidden hover:scale-[1.02] transition-all duration-300 ease-in-out">
            ...
            <div className="flex flex-col gap-2 py-2 w-full h-[30%]">
                <div className="flex flex-row justify-between items-center font-bold w-full">
                    <div className="w-[50%] text-white text-[0.8rem]"><p>{name}</p></div>
                    <span className="text-[0.75rem] text-gray-500 block px-3 py-1 bg-[#BEE3CC] rounded-md">{price}</span>
                </div>
                <div className="flex flex-row justify-between w-full">
                    <div className="text-gray-700 text-[0.8rem] w-[50%] flex flex-row">
                    ...
                    {!inStock && (
                        <div className="text-[#ED735D] text-base text-right font-bold">
                            sold out
                        </div>
                    )}
                </div>
